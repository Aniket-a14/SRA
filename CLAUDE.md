...
3. **Orchestration** (`backend/src/services/analysisService.js`, single large function — this is the file to read to understand the whole pipeline):
   ...
   - **Reflection loop** (max 2 passes, quality threshold 85): `ReviewerAgent` (approve/reject) + `CriticAgent` (6Cs score) gate the draft. On failure, `DeveloperAgent.refineSRS` does a **surgical** refinement of only the flagged section (Shell/Features/Requirements/Appendices) rather than a full regeneration — feedback keyword-matching decides which section is targeted. **A refinement is only as surgical as its schema.** Whatever `refineSRS` returns is spread straight over the draft, so `REFINEMENT_SCHEMAS` must have an entry for every section the loop can target, and an unmapped one throws rather than falling back to `SRSSchema` — Appendices did fall back, which meant the model was shown one section, asked for a whole SRS, and its answer overwrote everything it had not rewritten. Appendices is also the branch selected most often (`hasAppendicesFeedback` is tested first and matches any feedback mentioning a diagram), so finished documents came out with no diagrams at all. Schema shape is necessary but not sufficient — `analysisModels` is not a required sub-field — so `mergeRefinedAppendices` replaces a diagram only with one that has code.
