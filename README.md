...
[![Socket Badge](https://badge.socket.dev/npm/package/@sra-srs/sra-cli/4.2.2)](https://badge.socket.dev/npm/package/@sra-srs/sra-cli/4.2.2)
...
| **AI (models)** | `GEMINI_MODEL_NAME`, `GEMINI_UTILITY_MODEL_NAME`, `GEMINI_EMBEDDING_MODEL`, `GEMINI_EMBEDDING_DIMENSIONS` | Yes (embeddings pair), otherwise default fallbacks | No model id is hardcoded in `src/` — read from the environment via `backend/src/config/models.js`, so a retired model is an env edit plus a restart. `GEMINI_EMBEDDING_DIMENSIONS` must equal the `vector(N)` width in `schema.prisma`. Gemini is the only provider with a server-side default: the platform runs it for embeddings and internal utility calls. There is no `OPENAI_MODEL_NAME`/`CLAUDE_MODEL_NAME`/`GROK_MODEL_NAME` here — those are BYOK-only and must come from the user's own request. |
