...
3. **Automated PR Path-Based Labeler**: A high-efficiency classification workflow (`pr-labeler.yml` + `.github/labeler.yml`) parses changed file scopes on incoming Pull Requests and dynamically tags them (e.g., `backend`, `frontend`, `cli`, `ci`, `docs`) to ensure smooth visual organization and review delegation.
...
8.  **Multi-Provider Adapter Registry (`services/providers/`)**: `BaseAgent` and `ChatAgent` no longer talk to Gemini directly — every call is routed through a shared adapter interface (`generateContent`, `generateContentStream`, `countTokens`, `classifyError`) implemented per provider:
    - `GeminiAdapter`, `OpenAIAdapter`, `ClaudeAdapter`, `GrokAdapter` — all **bring-your-own-key (BYOK)**. Each requires a user-supplied key for generation, and there is no silent fallback to the platform key for any of them, Gemini included.
    - The platform's own `GEMINI_API_KEY` funds exactly one thing: **embeddings**. `embeddingService.js` stays fixed to Gemini regardless of which provider generates the document, because the `vector(768)` pgvector column is dimensioned to that embedding model and mixing providers within one index corrupts similarity scores silently.
