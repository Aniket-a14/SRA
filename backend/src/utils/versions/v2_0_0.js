...
Focus on "What" and "Why".
    ...
    personaInstruction = `
You are a Principal Systems Engineer focused on Product Architecture and Technical Viability.
Your requirements must emphasize the holistic product ecosystem, including the hardware-software boundary, subsystem interactions, and overall system integrity.
      `;
...
`;
  ...
  const descriptionVsNameSeparation = `
================================================================
...
2. MANDATORY SECTION SEGMENTATION
   ...
   - Segregate concerns: System Architecture | Subsystems | Data Persistence | Connectivity | Regulatory/Safety.

3. SELECTIVE KEYWORD BOLDING ONLY
   - You may ONLY use markdown bolding (**word**) for:
     * Product Name (first occurrence per section)
     * Subsystem Names (e.g., **Core Processor**, **Physical Interface**, **Data Engine**)
     * Environment Components (e.g., **Primary Database**, **Local Buffer**, **Remote API**)
     * Critical Interfaces (e.g., **User Display**, **Hardware Sensor**)
...
`;
  ...
  const ieeeDetailedGuidelinesVerbatim = `
================================================================
...
- description: 4.x.1 Description and Priority: Provide a short description of the feature and indicate whether it is of High, Medium, or Low priority. You could also include specific priority component ratings, such as benefit, penalty, cost, and risk (each rated on a relative scale from a low of 1 to a high of 9).
...
[7] GLOSSARY: Define all the terms necessary to properly interpret the SRS, including acronyms and abbreviations. You may wish to build a separate glossary that spans multiple projects or the entire organization, and just include terms specific to a single project in each SRS.
...
- tbdList: Collect a numbered list of the TBD (to be determined) references that remain in the SRS so they can be tracked to closure.
`;

  const technicalRenderingRulesVerbatim = `
STRICT RULES:
...
<task>
Read the raw input (a JSON Array of Strings). RECONSTRUCT the text. The word immediately following "Project:" is the PROJECT NAME.
ANALYZE core modules, user roles, constraints, and business goals.
EXTRACT & CATEGORIZE into IEEE sections (Introduction, Description, Features, NFRs).
GENERATE missing artifacts and format using strict rules.
</task>
...
${nameGovernance}
${identifierGovernance}
${quantityGovernance}
${descriptionVsNameSeparation}
${technologyAgnosticism}
${narrativeDiscipline}

${formattingInstruction}

${ieeeDetailedGuidelinesVerbatim}

${technicalRenderingRulesVerbatim}

[DIAGRAM AUTHORITY INJECT]
${diagramAuthority}
</constraints>
