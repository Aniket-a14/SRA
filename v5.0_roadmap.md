...
## 🏛️ Pillar 1: Sovereign Inference Engine (Local LLM Integration)
**Goal**: Completely replace Google Gemini APIs with self-hosted open-weights models without sacrificing output quality.

### Current State (v4.x — Cloud)
*   `backend/src/services/aiService.js` supports **two providers**: `google` (Gemini 2.5 Flash — default) and `openai` (OpenAI-compatible API).
*   `backend/src/services/agents/BaseAgent.js` — all 5 agents (Developer, Reviewer, Critic, ProductOwner, Architect) are hardcoded to `genAI` (Gemini SDK).
...
3.  **The Adapter Layer**: Add a `local` model provider to `backend/src/services/aiService.js` that routes API calls to the local inference server. Refactor `BaseAgent.js` to accept provider config instead of hardcoding `genAI`.

### Migration Checklist
- [ ] Add `local` provider branch in `aiService.js` (OpenAI-compatible client → vLLM/Ollama)
...
## 🔗 Pillar 2: Local AI Agents & Deterministic JSON
...
*   **5-Agent Architecture** operational: `DeveloperAgent`, `ReviewerAgent`, `CriticAgent`, `ProductOwnerAgent`, `ArchitectAgent` — all extend `BaseAgent`.
...
## 🤝 Pillar 3: Private RAG & Offline Document Intelligence
**Goal**: Transition vector embeddings and RAG from cloud dependency to 100% offline document retrieval.

### Current State (✅ Functional — Cloud-Bound)
*   **Embedding Model**: `backend/src/services/embeddingService.js` uses `gemini-embedding-001` (Google Cloud, 768 dimensions).
...
*   **Graph RAG**: `backend/src/services/ragService.js` combines vector search with `graphService.js` entity traversal for relationship-aware context retrieval.
*   **RAG Evaluation**: `backend/src/services/evalService.js` implements RAGAS-style scoring (Faithfulness, Context Precision, Answer Relevancy) using an LLM-as-judge pattern.
*   **Context Injection**: `backend/src/services/aiService.js` automatically injects RAG context into the system prompt with truncation caps (~8000 chars).
...
### Migration Checklist
- [ ] Swap `gemini-embedding-001` → local model in `embeddingService.js`
...
## 🛡️ Pillar 4: The Data Flywheel & High-Performance Fine-Tuning
...
*   **SFT Formatting**: Pipeline converts raw documents into instruction/input/output JSONL format → **70 training samples**, **9 test samples**.
*   **Token Analysis**: Average sample is **~32,260 tokens**. Distribution: 84.3% > 16k tokens, 28.6% > 32k tokens, 5.7% > 64k tokens.
*   **Quality Filtering**: Samples exceeding **65,536 tokens** are automatically pruned → **66 training samples** retained (4 removed), all 9 test samples kept.
...
*   **Max Sequence Length**: **65,536 tokens** — matching the filtering ceiling to eliminate truncation.
...
| `merge_and_format_dataset.py` | Merge raw JSON → split into train/test JSONL |
...
## 📈 Pillar 5: The MLOps Resource & Observability Layer
**Goal**: Orchestrate local vs. Lab hardware and monitor inference health.

### Current State (✅ Partially Implemented)
*   **Job Queue**: `backend/src/services/queueService.js` uses **Upstash QStash** (cloud) for async analysis jobs with idempotency checks. Has `MOCK_QSTASH` fallback for local dev.
*   **Redis**: `backend/src/config/redis.js` configured with **ioredis** + Upstash TLS. Used for rate limiting & caching.
*   **Docker Compose**: Production-ready with **Nginx reverse proxy** (load balancer), scalable backend replicas, and Next.js standalone frontend.
*   **RAG Evaluation**: `backend/src/services/evalService.js` runs automated RAGAS-style quality checks.
*   **Logging**: Structured JSON logging via Pino (`backend/src/utils/logger.js`).
...
*   **Queue Migration**: Replace Upstash QStash with **BullMQ** (Redis-based) for fully local, self-hosted job processing.
*   **Tracing & Rate Limiting**: Implement VRAM-aware rate limiting to handle heavy multi-agent reflection loops.
*   **Evaluations (CI/CD for AI)**: Automated benchmarking of every new model checkpoint against the "6Cs" quality metric.

### Migration Checklist
- [ ] Create `docker-compose.gpu.yml` with NVIDIA runtime, vLLM container, and GPU limits
- [ ] Replace `queueService.js` Upstash QStash → BullMQ + local Redis
...
| **Training Data** | N/A | 79 SRS docs → 66 filtered SFT samples |
...
| :--- | :--- | :---: | :--- |
| **Phase 1** | **The Adapter** | ⬜ | Add `local` provider to `aiService.js` + refactor `BaseAgent.js` for provider-agnostic routing. |
| **Phase 2** | **The Grammar** | ⬜ | Implement structured output constraints / grammar for local model JSON compliance. |
| **Phase 3** | **The Private Node** | ⬜ | Swap `embeddingService.js` to local models, re-embed knowledge chunks, containerize pgvector. |
| **Phase 4** | **The Lab Training** | 🟡 | Data pipeline complete. Training script ready. Pending first fine-tuning run on lab hardware. |
| **Phase 5** | **The Queue** | ⬜ | Migrate `queueService.js` from Upstash QStash to BullMQ. Create `docker-compose.gpu.yml`. |
| **Phase 6** | **The Switchover** | ⬜ | Cut Gemini access completely. Route all generation + embedding to SRA-Pro local stack. |
...
| Training Script | `train_sra.py` — ready for lab deployment |
...
| Docker Services | 4 (Nginx LB, Backend ×2, Frontend, CLI) |
